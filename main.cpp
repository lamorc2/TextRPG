...
bool Move(Location* loc, Direction dir){
...
void showInventoryMenu(Player& player) {
    ...
    if(entry.item->getType() == "Weapon"){//weapon

    }else {
    entry.item->use();
    if(entry.item->getType() == "Doobie"){//doobie
    	player.mana += entry.item->getMana();
    	if(player.max_mana < player.mana){
    		player.mana = player.max_mana
    		std::cout << "You now have " << player.mana << "/" << player.max_mana << " mana!" << std::endl;
    	}
    }else{//drink
    	player.health += entry.item->getHealthRestored();
    	if(player.max_health < player.health){
    		player.health = player.max_health
    		std::cout << "You now have " << RED <<player.health << "/" << player.max_health << RESET << " HP!" << std::endl;
    	}
    }
    entry.quantity--;
    if (entry.quantity <= 0) {
...
void CadenIntro(Player& player){
	std::cout << "\n\n You pull up to your house, the sound of music filling the air as you roll into the driveway. You take one look through the
	...
	std::cout << "You hear the music and feel the bass shaking the walls as you open the door and everyone turns your way."
...
void Combat(Player& player, std::vector<Enemy> enemies, bool surprise){//Combat: Takes player, list of enemies , bool surprise // surprise is true if enemies go first
	...
	while(player.health > 0 && !enemies.empty()){ // While Player is alive and 
