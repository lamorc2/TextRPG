...
void takeAction(Player& target, Enemy& enemy, Action* action){//enum class ActionType {Heal, Buff, Debuff, Attack, manaRestore, dmgSpell }; 
	std::cout << "The "<< enemy->name << " uses " << action->name << "." << std::endl;
	enemy.mana -= action->manaCost;

	switch(action->type):
		case ActionType::Heal:
			enemy->restoreHealth(action->stat_value);
			std::cout << enemy->name << "restores " << stat_value << "HP."
			break;

		case ActionType::Debuff: 
			target->debuff() //TODO : make debuff function + counter for player
			std::cout << "You feel your arms grow heavy, you lose " << stat_value << " strength. " << std::endl;
			break;

		case ActionType::Attack:
			target->takeDamage(enemy->damage);
			std::cout << "You take " << enemy->damage << "damage. You have " 
			break;

		case ActionType::dmgSpell:
			target->takeDamage(action->stat_value);
			break;


		case ActionType::manaRestore:
			enemy->manaRestore(action->stat_value);
			break;

		default:
			std::string error = "ActionType not recognized for action: " + action->name;
			Logger::Log(error);
			break;

		if(action->statusEffect != StatusEffect::None){
			//TODO : Implement status effect for player + status check in combat function
...
void GameOver(Player& player){// add option to quit or load save file 
	std::cout << BOLD << "=== GAME OVER ===" << RESET << std::endl;
}
...
bool Load(){// Return false if load is unsuccessful/file is empty, force new game 
	//NOTE: when loading location data, check if enemies in room. If it's false, clear enemy vector. then call enter function at end of load
}
...
void CadenIntro(Player& player){
	...
	typewriterPrint("Logan: Oh yeah, he was getting work done. Unlike Cody, you unemployed bastard!");
	std::cout << "He points an accusatory finger at Cody, who starts wagging his tail upon hearing his name" << std::endl;
	typewriterPrint("Dylan: C’mon, you're scaring poor Cody.");
	typewriterPrint("Logan: Nah, only thing that scares him is a job application.");
	typewriterPrint("Caden (You): Forget about Cody, someone pour me a cup of that punch!");
	std::cout << "You glance at the kitchen sink and spot the empty glass pitcher with its plain white label. A few iridescent droplets cling to the bottom, shifting colors under the kitchen light." << std::endl;
	typewriterPrint("Connor: Sorry buddy, we just polished it off a couple minutes ago... blame Dylan.");
	typewriterPrint("Dylan: Blame me? You had like 3 cups!");
	typewriterPrint("Tim: NAH, I saw you downing the rest of it straight out of the pitcher.");
	typewriterPrint("Dylan: Alright, maybe I did, but that was just my fair share.");
	typewriterPrint("Cayden: You're lucky anyways -- that shit tasted like Skittles and gasoline");
	typewriterPrint("Connor: Hey, two litres for $10 is a deal, no matter how nasty it is");
	typewriterPrint("Rylan: Yeah, okay, whatever you say buddy.");
	typewriterPrint("Connor: It doesn't matter-");
	std::cout << "Connor grabs a can from the fridge and cracks it open with a grin." << std::endl;
...
int main(int argc, char[] argv){
	...
	const bool canStack = true;
	const bool noStack = false;
	...
	ItemDatabase["busch_apple"] = std::make_unique<Item>(ItemType::Drink, 999,RED + "Busch Light Apple" + RESET, "A rare, limited-edition classic. They only come around once a year. Restores all HP", canStack);
	...
	ItemDatabase["preroll"] std::make_unique<Item>(ItemType::Doobie, 25, "Pre-Roll", "A basic preroll. +25 Mana", canStack);
	...
	LocationDatabse["living_room"]
	LocationDatabase["porch"]
	LocationDatabase["magic-door"]
	LocationDatabase[]
	...
	////initialize enemies //Enemy(const std::string& name_,  int max_HP,  int atk, int mana_=0);

	EnemyDatabase["goblin"] = Enemy("Goblin",20,15);


	////

	///Build Locations [add enemies, add connected locations, add interactables]
