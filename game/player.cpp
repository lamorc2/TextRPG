...
void Player::restoreMana(const int amnt){
    mana += amnt;
    if(mana > max_Mana){
        mana = max_mana;
    }
...
void Player::to_json(json& j, const Player& p) {
    j = json{{"name", p.name}, {"hp", p.health}, {"max_hp", p.max_health}, {"mana", p.mana}, {"max_mana", p.max_mana}, {"strength", p.strength}, {"level", p.lvl}, {"inventory", p.inventory}};
};
